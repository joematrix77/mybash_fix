...
# mybash installer (patched fork) — fixes "Beautiful Bash" on modern distros.
#
...
#      it, so prompt/fastfetch icons rendered as blank boxes — the #1 reason
#      people think the install "didn't work".
#   2. Installs JetBrainsMono Nerd Font (icons render cleanly, unlike the
#      cramped "Mono" font variants).
...
#      so your own settings survive installs, re-runs and `git pull`s.
#   4. Always links config from the complete clone and removes dangling
#      symlinks safely (the bug that could leave a broken ~/.bashrc).
#
...
REPO_URL="https://github.com/joematrix77/mybash_fix"
...
setup_directories() {
    ...
    if [ -d "$MYBASHDIR" ]; then
        ...
        if ! git clone "$REPO_URL" "$MYBASHDIR"; then
